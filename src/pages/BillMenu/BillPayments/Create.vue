<template>
  <div class="col-md-12">
    ...
    />
    <h2 class="text-center">{{ $t('billpayments.index') }}</h2>
    <card>
        ...
        <ValidationObserver v-slot="{ handleSubmit }">
          <form class="form-horizontal" @submit.prevent="handleSubmit()">
           ...
           <div class="col-ms-12 col-md-6">
              ...
              <div class="col-sm-10">
                ...
                >
                  <base-input
                    ...
                    :error="errors[0]"
                     @input="if(payment.total > facturas.totalPendiente){payment.total = facturas.totalPendiente}"
                    type="number"
                    :class="[
                      { 'has-success': passed },
                      { 'has-danger': failed }
            ...
            <div class="row d-flex justify-content-center" v-if="facturas.pendiente>0">
             <router-link to="/billpayments/index">
           ...
           <div class="col-ms-12 col-md-6">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-4">
                <label class="pt-2"> {{ facturas.sucursales.nombre}}</label>              
                </div>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-sm-4">
                <label class="col-form-label">{{ facturas.comprobante }}</label>
                </div>
              </div>
              <div class="row">
                <label class="col-sm-2 col-form-label">Sub Total</label>
                <div class="col-sm-4">
                <label class="col-form-label">{{ facturas.subTotal }}</label>
                </div>
              </div>
              <div class="row">
                <label class="col-sm-2 col-form-label">Itbis</label>
                <div class="col-sm-4">
                <label class="col-form-label">{{ facturas.itbis }}</label>
                </div>
              </div>
              ...
              <div class="row">
                <label class="col-sm-2 col-form-label">Total</label>
                <div class="col-sm-4">
                <label class="col-form-label">{{ facturas.total }}</label>
                </div>
              </div>
              <div class="row">
                <label class="col-sm-2 col-form-label">Abono</label>
                <div class="col-sm-4">
                <label class="col-form-label">{{ facturas.abono }}</label>
                </div>
              </div>
              <div class="row">
                <label class="col-sm-2 col-form-label">Saldo Pendiente</label>
                <div class="col-sm-4">
                <label class="col-form-label">{{ facturas.pendiente }}</label>
                </div>
              </div>
              <hr />
            </div>
...
export default {
 ...
 async mounted() {
  ...
  methods: {
   async  checkId() {
   await axios
        .get(this.baseApiUrl + 'Facturas/' + this.id)
        .then((response) => {
          this.isLoading = true
        ...
        await axios
        .get(this.baseApiUrl + 'PagosFacturas/ByFactura/' + this.id)
        .then((response) => {
          this.isLoading = true
   ...
   await axios
    ...
    reload(){
      axios
        .get(this.baseApiUrl + 'PagosFacturas/ByFactura/'+this.payment.facturaId)
        .then((response) => {
        this.tableData=response.data
        })
        .catch((error) => {
      ...
      if (this.validateFields()) {
        ...
        axios
          ...
          .then((response) => {
            ...
            this.clear()
            this.$router.push({ path: '/billpayments/index' })
          })
          .catch((error) => {
    ...
    create() {
      if (this.validateFields()) {
        ...
        this.isLoading = true
          axios
          .post(this.baseApiUrl + 'PagosFacturas', this.payment)
          .then((response) => {
            this.globalSweetMessage(response.data.message)
            this.facturas.pendiente=response.data.result.facturas.pendiente
            this.facturas.abono=response.data.result.facturas.abono
            this.clear()
            this.reload()
          })
          .catch((error) => {
